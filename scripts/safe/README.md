...
cp scripts/safe/parameters.json.example scripts/safe/parameters.json
```
...
npx hardhat run scripts/safe/manageOwners.ts --network mainnet

# For arbitrary transactions (any contract call)
npx hardhat run scripts/safe/prepareTransaction.ts --network mainnet
```
...
```bash
# First owner
SIGNER_INDEX=0 npx hardhat run scripts/safe/signSafeTransaction.ts --network mainnet

# Second owner (on their machine or with different key)
SIGNER_INDEX=1 npx hardhat run scripts/safe/signSafeTransaction.ts --network mainnet
...
```bash
npx hardhat run scripts/safe/executeSafeTransaction.ts --network mainnet
```
...
```bash
# Configure
cat > scripts/safe/parameters.json << 'EOF'
{
