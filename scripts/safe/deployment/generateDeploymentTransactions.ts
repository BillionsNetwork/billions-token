...
async function main() {
    ...
    const safeAddress = safeConfig.safeAddress;
    ...
    const initData = BillionsNetworkToken.interface.encodeFunctionData(initFragment!, [
        TOKEN_NAME,
        TOKEN_SYMBOL,
        safeAddress, // Initial owner of the token logic (OwnableUpgradeable)
        TOTAL_SUPPLY,
    ]);
    ...
    console.log(`\nSaved ${transactions.length} transactions to output-deployment-transactions.json`);
}
